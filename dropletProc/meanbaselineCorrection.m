function tracks=meanbaselineCorrection(tracks,colNames)
%{
...
%}
    ...
    if length(colNames)==1    
        for n =1:size(tracks)
            ...
            eval(['tracks(',num2str(n),').Feat.',colNames{1},'_base=ntmp;']);
        end
    else
        for n =1:size(tracks)
            eval(['tmpSig=tracks(',num2str(n),').Feat.',colNames{2},';']);
            ntmp=tmpSig-f_y(1:length(tmpSig))';
            eval(['tracks(',num2str(n),').Feat.',colNames{2},'_base=ntmp;']);
        end
    end
end
