function tracks=linearBaselineCorrection4(tracks, osci_table,colNames)
...
Description of function
    ...
    Corrected signal values are also normalized with a mean value of individual trough values.
    ...
    if length(colNames)==1    
        for n =1:size(tracks)
            ...
            if length(trough_v) >= 2                
                ...
                est_val4=est_val./mean_ctv;
               
            else
                est_val=NaN*ones(length(tp),1);
                est_b=NaN*ones(length(tp),1);
            end
            eval(['tracks(',num2str(n),').Feat.',colNames{1},'_linear4=est_val4;']);
